...
// Mapeia o tipo de interação para a chave de prefixos exportada pelos módulos.
// (Bug corrigido: antes era montado como `${tipo}Prefixes`, que gerava
//  "buttonsPrefixes"/"modalsPrefixes" — nomes que não existem nos módulos,
//  fazendo botões como "aprovar_123" nunca encontrarem seu handler.)
const CHAVES_PREFIXO = {
    buttons: "buttonPrefixes",
    selects: "selectPrefixes",
    modals: "modalPrefixes"
};
...
function encontrarHandler(tipo, customId) {
    ...
    const chavePrefixos = CHAVES_PREFIXO[tipo];
    for (const mod of modules) {
    ...
    console.warn(`⚠️ Nenhum handler encontrado para ${tipo}: "${customId}"`);
    return null;
}
...
client.on("interactionCreate", async (interaction) => {
    try {
        ...
        const msgErro = { content: "❌ Ocorreu um erro ao processar essa ação.", flags: MessageFlags.Ephemeral };
